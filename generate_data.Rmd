...
It is probably a safe assumption that (at least for subtyping these tumors), the latent variables with high variance across the cohort are the most "interesting." Let's look only at those with a standard deviation of 0.075 or greater (green in the above plot).
...
mp_res_tidy_filt %>% 
  ...
  write_feather("filt_nf_mp_res.feather")
plier_loadings_df %>% write_feather("mp_loadings.feather")
...
write_feather(tidy_loadings, "mp_loadings_tidy.feather")
...
write_feather(drug_targets, 'dtex_targets.feather')
